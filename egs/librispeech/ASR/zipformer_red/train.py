...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Scale for accuracy-prediction loss."
    )
...
def get_model(params: AttributeDict) -> nn.Module:
    ...
    encoder_subsample = frame_reduce.ReduceFrames(num_channels=_to_int_tuple(params.encoder_dim)[0],
                                                  max_length_proportion=0.75,
                                                  score_stddev=3.0)
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        simple_loss, pruned_loss, ctc_loss, pred_loss = model(
            x=feature,
            x_lens=feature_lens,
            y=y,
        ...
        loss += params.pred_loss_scale * pred_loss * num_frames
    ...
    info["pred_loss"] = pred_loss.detach().cpu().item()
